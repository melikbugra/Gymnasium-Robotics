...
def register_robotics_envs():
    ...
    for reward_type in ["sparse", "dense"]:
        ...
        register(
            id=f"FetchPlaceInBox{suffix}-v1",
            entry_point="gymnasium_robotics.envs.fetch.place_in_box:MujocoFetchPlaceInBoxEnv",
            kwargs=kwargs,
            max_episode_steps=200,  # PlaceInBox needs more steps (manual ~300, optimized ~150-200)
        )

        register(
            id=f"FetchPegInHole{suffix}-v1",
            entry_point="gymnasium_robotics.envs.fetch.peg_in_hole:MujocoFetchPegInHoleEnv",
            kwargs=kwargs,
            max_episode_steps=100,  # PegInHole assembly task
        )
